...
def interpolate_lambda_MicrOMEGAs(mass_val,Omega_val,QCDmodel):
    '''
    This script calculates the lambda_HS parameter for a given relic density (Omega h^2) and dark matter mass
    mass_val : dark matter mass value in GeV
    Omega_val: relic density (Omega h^2)
...
def DMspectra_inttable(DMmass_val,lambdahs_val,particle,smooth):
    '''
...
def tominimize_DD(Lambda_val,DMmass,Lambda_vec,Mass_vec,csi_vec,Exp):
    #print(Lambda_val)
...
def minimize(DMmass,Gamma_H,Gamma_inv_measured):
    lambda_0 = 0.01
